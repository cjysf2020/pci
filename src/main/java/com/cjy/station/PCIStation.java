...
public class PCIStation {
    ...
    private Dictionary<String, Double> transfers = new Hashtable<String, Double>();
    ...
    public void initStations(){
        ...
        for(Dictionary<String, String> row: hxrs){
            ...
            double num = Double.parseDouble(row.get(paramcols[3]));
    ...
    public void calcInOut(String[] timespan, Stations st){
        ...
        double outCount = 0;
        for(Dictionary<String, String> row: rs){
            double icount = Integer.parseInt(row.get(columns[0]));
        ...
        double ti = (inCount / inMax);
        double to = (outCount / outMax);
        tmpres[0] = ti;
        tmpres[1] = to;
        ////////
    ...
    public void calcWaitPerson(String[] timespan, Stations st){
        ...
        double upWaitMax = Integer.parseInt(st.getFLOW_WAIT_UP_MAX());
        double downWaitMax = Integer.parseInt(st.getFLOW_WAIT_DOWN_MAX());
        double[] tmpres = new double[2];
    ...
    public void calcTransfer(String[] timespan, Stations st){
        ...
        if(st.getSTATION_TYPE().equals("2")) {
            ...
            for(int i=0;i<tmpres.length;i++){
                ...
                Double maxNum = transfers.get(key);
                if(maxNum != null){
                    double d = (double)(tmp.getCount() / maxNum);
                    tmpres[i] = d;
                }else{
                    tmpres[i] = 0;
                }
